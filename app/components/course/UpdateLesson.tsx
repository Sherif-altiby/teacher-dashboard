...
import { X, Video, Link2, AlignRight, Loader2 } from "lucide-react";
...
const UpdateLesson = ({
    ...
    showUpdateLesson: (val: boolean) => void;
    lesson: any;
}) => {
    ...
    const { mutate, isPending } = useMutation({
        mutationFn: async () => {
            const response = await fetch(
                `${API}/teacher/update-lesson/${lesson?._id}`,
                {
                    method: "PUT",
                    headers: {
            ...
            if (!response.ok) {
                throw new Error(data?.message || "حدث خطأ أثناء تعديل الدرس");
            }
        ...
        onError: (error: any) => {
            toast.error(error.message || "حدث خطأ أثناء تعديل الدرس");
        },
    });
    ...
    return (
        <div className="fixed inset-0 z-100 flex items-center justify-center p-2">
            ...
            <div
                ...
                onClick={() => showUpdateLesson(false)}
            />

            {/* Modal Container */}
            <div
                className="relative w-full max-w-xl bg-white rounded-[2.5rem] shadow-2xl overflow-hidden animate-in zoom-in-95 duration-300"
                dir="rtl"
            >
                {/* Header */}
                ...
                <form onSubmit={handleSubmit} className="p-4 md:p-10 space-y-7">
                    ...
                    <div className="space-y-2">
                        ...
                        <div className="relative">
                            ...
                            <Video
                                className="absolute right-4 top-4 text-[#0066FF]"
                                size={20}
                            />
                        </div>
                    </div>
